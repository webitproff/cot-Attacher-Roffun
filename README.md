...
Плагин attacher - это расширение под Cotonti, с помощью которого можно загружать файлы, прикреплять к страницам статей, сообщениям на форуме, выводить в виде списка для скачивания,  галереи изображений, bb - кода, вставлять в визуальный редактор большую или маленькую миниатюру по клику. 
...
Эта функция применяется для получения информации об одном файле, прикрепленном к объекту. 

        $area - определяет для какого объекта выводить - код модуля / плагина.
        $item - ID объекта, параметры которого выводятся.
        $field - имя поля выводимого объекта (если для загрузчика указывали).
        $column - вернет значение указанного поля этого объекта.
