 ...
 * // переделано 03 Март 2026 конвертация картинок и загрузка по внешней ссылке
...
function att_ajax_handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
{
    ...
    // Новая обработка изображений: ресайз до 1200px по ширине и конвертация в WebP
    if ($is_img) {
        ...
        if ($src_image) {
            ...
            // Если ширина больше 1200, масштабируем
            if ($width > 1200) {
                $new_width = 1200;
            ...
            imagewebp($dst_image, $new_file_path, 85);
            ...
            // $is_img остается true
        }
        // Если не удалось загрузить изображение, оставляем оригинал без изменений (маловероятно)
    }
...
function att_ajax_handle_url_upload($url)
{
    ...
    // Далее – как при обычной загрузке (копируем логику из att_ajax_handle_file_upload)
    $is_img = in_array($file_ext, ['gif', 'jpg', 'jpeg', 'png', 'webp']);
    $file_path = ($cfg['plugin']['attacher']['folder'] ?? 'datas/attacher') . '/' . $area . '/' . $item . '/' . $file->name;
