 ...
 * Filename attacher.functions.php
 * @package Attacher
 * @author Roffun
...
Warning: imagecreatefromjpeg(): gd-jpeg: JPEG library reports unrecoverable error: Not a JPEG file: starts with 0x52 0x49 in /home/w/webitproff/funsmart.club/public_html/plugins/attacher/inc/attacher.functions.php on line 1058 Warning: imagecreatefromjpeg(): "attacher/mstore/300/att_502.jpg" is not a valid JPEG file in /home/w/webitproff/funsmart.club/public_html/plugins/attacher/inc/attacher.functions.php on line 1058 Warning: imagecreatefromjpeg(): gd-jpeg: JPEG library reports unrecoverable error: Not a JPEG file: starts with 0x52 0x49 in /home/w/webitproff/funsmart.club/public_html/plugins/attacher/inc/attacher.functions.php on line 1058 Warning: imagecreatefromjpeg(): "attacher/mstore/300/att_502.jpg" is not a valid JPEG file in /home/w/webitproff/funsmart.club/public_html/plugins/attacher/inc/attacher.functions.php on line 1058  */
/**
 * Creates image thumbnail.
 *
 * @param string $source Original image path.
...
function att_filebox($area, $item, $field = '', $type = 'all', $limit = -1, $tpl = 'attacher.filebox')
{
    ...
    if (!$jQtemlatesOut) {
        ...
        // The Load Image plugin is included for the preview images and image resizing functionality
        Resources::linkFileFooter($cfg['plugins_dir'] . '/attacher/lib/JavaScript-Load-Image/js/load-image-fetch.js', 'js');
...
{
    global $db, $db_attacher, $att_item_cache;
    ...
    // Используем значения по умолчанию из конфигурации, если параметры отсутствуют
    $width = isset($params['width']) ? (int)$params['width'] : $plu['thumb_x'];
    $height = isset($params['height']) ? (int)$params['height'] : $plu['thumb_y'];
    $frame = isset($params['frame']) ? $params['frame'] : $plu['thumb_framing'];

    $src = att_thumb($params['id'], $width, $height, $frame);
    $url = att_thumb($params['id'], $plu['thumb_big_width'], $plu['thumb_big_height'], $plu['thumb_big_framing'], true);
    ...
    if ($plu['thumb_clickable'] === 'clickable') {
        $title = $plu['thumb_alt_to_title'] && !empty($params['alt']) ? ' title="' . htmlspecialchars($params['alt']) . '"' : '';
        $snippet = '<a href="' . $url . '"' . $title . '>' . $img . '</a>';
    }

    if ($plu['thumb_wrapper']) {
        $thumb_wrapper = !empty($plu['thumb_wrapper_class']) ? ' class="' . $plu['thumb_wrapper_class'] . '"' : '';
        $snippet = '<span' . $thumb_wrapper . ' data-snippet="attacher">' .$snippet. '</span>';
    }
    return $snippet;
}